...
from .icp_tools import best_fit_transform, nearest_neighbor_cuda
...
class ICP_Cuda:
    ...
    @time_record
    def icp(self, max_iterations: int = 100, tolerance: float = 1e-6, verbose: bool = True, threads_per_block: int = None) -> Tuple[np.ndarray, List[float], int]:
        A = self.source
        B = self.target
        ...
        for i in range(max_iterations):
            self.distances, indices = nearest_neighbor_cuda(src3d.T, B, threads_per_block)
